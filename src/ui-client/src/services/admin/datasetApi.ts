...
const encounter: AdminDatasetQuery = {
    ...
    sqlStatement: "SELECT      personId = CONVERT(NVARCHAR(10),e.person_id)	, encounterId = CONVERT(NVARCHAR(10),e.visit_occurrence_id)	, admitDate = e.visit_start_date    , class = e.visit_type	, dischargeDate = e.visit_end_date	, [location] = 'Unknown'	, [status] = 'Unknown'  FROM dbo.v_encounter e  WHERE e.visit_type_code IN ('IP','OP')",
    tags: []
};
const platelet: AdminDatasetQuery = {
    ...
    sqlStatement: "SELECT personId      = CAST(SUBJECT_ID AS NVARCHAR), encounterId   = CAST(HADM_ID AS NVARCHAR), category      = 'lab', code          = LOINC_CODE, effectiveDate = DATEADD(YEAR,-150,CAST(CHARTTIME AS DATETIME))  , valueString   = VALUE, valueQuantity = VALUENUM, valueUnit     = VALUEUOM FROM [dbo].[v_LABEVENTS] WHERE LABEL = 'Platelet Count'",
    tags: []
};
const procedure: AdminDatasetQuery = {
    ...
    sqlStatement: "SELECT      personId = CONVERT(NVARCHAR(10),p.person_id)	, encounterId = CONVERT(NVARCHAR(10),p.visit_occurrence_id)	, category = 'procedure'	, code = p.procedure_source_value	, coding = p.procedure_vocabulary_id	, performedDateTime = p.procedure_date	, [text] = p.[procedure_name]FROM dbo.v_procedure p",
    tags: []
};
