...
2) Populate the database `tables`, `stored procedures`, and `functions` using the [LeafDB.Schema.sql](https://github.com/uwrit/leaf/blob/master/src/db/build/LeafDB.Schema.sql) script. Be sure to preface the script with `USE <my_db_name> GO` before executing.
